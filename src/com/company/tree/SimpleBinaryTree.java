...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    public void removeTopLeaves() {
        final int[] maxLevel = {0};
        BinaryTreeAlgorithms.preOrderVisit(root, (value, level) -> {
            if (level >= maxLevel[0]) {
                maxLevel[0] = level;
            }
        });
        this.depth = maxLevel[0];
        removeLeft(root.left, root, 1);
        removeRight(root.right, root, 1);
    }

    private  void removeLeft(SimpleTreeNode node, SimpleTreeNode prevNode, int depth) {
        if (prevNode.left != null) {
            ...
            if (node.left != null ) {
                removeLeft(node.left, prevNode.left, depth);
            }
            if (node.right != null) {
                removeRight(node.right, prevNode.left, depth);
            }
        }
    }

    private  void removeRight(SimpleTreeNode node, SimpleTreeNode prevNode,  int depth) {
        if (prevNode.right != null) {
            if (node.right == null && node.left == null && depth != this.depth) {
                prevNode.right = null;
            }
            if (node.left !=null || node.right!=null) {
                depth++;
            ...
            if (node.left != null) {
                removeLeft(node.left, prevNode.right, depth);
            }
            if (node.right != null) {
                removeRight(node.right, prevNode.right, depth);
            }
        }
    }
}
